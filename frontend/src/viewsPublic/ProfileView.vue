...
import {
  ...
  mdiAlertCircle,
  mdiCheckCircle,
  mdiTicketConfirmation,        // ⟨NEW⟩ icon for redeem input
} from '@mdi/js'
...
import {
  ...
  me as apiMe,
  redeemCode as apiRedeemCode,   // ⟨NEW⟩
...
async function submitRedeemCode() {
  ...
  if (!code) {
    rdError.value = 'Please enter a code'
    return
  }
  ...
  try {
    ...
    // Success message based on function applied
    const fn = res?.function === 'is_admin' ? 'Admin' : res?.function === 'is_premium' ? 'Premium' : 'Feature'
    rdSuccess.value = `${fn} unlocked successfully`
    rdForm.code = ''

    // Refresh profile so flags show up immediately (e.g., auth.user.is_premium)
    try {
      const profile = await apiMe()
      auth.setUser(profile)
    } catch (_) { /* ignore refresh errors */ }
  } catch (err) {
    rdError.value = err?.message || 'Redeem failed'
  } finally {
    ...
    <SectionMain>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="space-y-6">
          ...
          <!-- ⟨NEW⟩ Redeem Code -->
          <CardBox is-form @submit.prevent="submitRedeemCode">
            <!-- messages -->
