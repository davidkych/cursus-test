...
───────────────────────
• Radio switch decides **Time** vs **Period** unequivocally:  
  only the active field keeps a *name* attribute, so the server never
  receives both.  
• Period mode now shows two time pickers whose **defaults are the
  current HK time** (start = now, end = now).  
• Minor JS tightening (no duplicate `required`, better name toggling).
"""
...
def availability_form(request: Request):
    """Interactive browser form for the availability endpoint."""
