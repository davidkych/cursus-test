...
Hot-fix 3-d-1 (2025-06-26)
─────────────────────────
`最近其它運動場狀態` showed “無資料” for every venue because the earlier
patch threw away the *nearest* arrays when converting the rapid-JSON
into a simple name-map.  We now keep the **entire** rapid array while
still caching the name-map.

Only two small JS changes were needed:

1. `fetchRapid()`  
   * keeps the full rapid array in a new global `rapidData` and builds
   *the map* from it.

2. `fetchNearest()`  
   * looks up the neighbour list from `rapidData`, so it once again
   gets the correct list and renders it in order.
"""
...
<script>
const id = "{esc_id}";        // original URL id (may be 1060a/b)
...
function nameFor(code) {{
  ...
  if (hard) return hard.replace(/\\(.+\\)$/, "");   // strip “(code)”
...
async function fetchRapid() {{
  ...
  rapidData = await res.json();                      // keep full array
...
/* ---------- 4-hour outlook block (unchanged) ---------------------- */
